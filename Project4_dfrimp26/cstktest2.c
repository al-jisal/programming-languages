...
char *account_toString(void *v) {
    ...
    int balanceLength = snprintf(NULL, 0, "%d", account->balance);
    int fullLength = strlen(account->name) + balanceLength + 2 + 1;

    char *output = malloc(fullLength * sizeof(char));
    if (output != NULL) {
        // Format the string "<name>: <balance>"
        snprintf(output, fullLength, "%s: %d", account->name, account->balance);
    }
    return output;
}
