...
void markAndSweep(ProgramState *state) {
    // Unmark all the heap chunks
    for (int i = 0; i < state->num_heap_chunks; i++)
    {
        state->heap[i]->marked = 0;
    }

    // Create a boolean array to mark reachable chunks
    bool *marked = (bool *)calloc(state->num_heap_chunks, sizeof(bool));

    // Mark all chunks reachable from the stack
    for (int i = 0; i < state->num_vars_on_stack; i++)
    {
        if (state->stack[i].reference != NULL)
        {
            markReachableChunks(state, state->stack[i].reference, marked);
        }
    }

    // Report reachable and garbage chunks
    printf("Reachable chunks:\n");
    for (int i = 0; i < state->num_heap_chunks; i++)
    {
        if (marked[i])
        {
    ...
    // Sweep phase: Free the garbage chunks
    printf("\nGarbage chunks:\n");
    for (int i = 0; i < state->num_heap_chunks; i++)
    {
        if (!marked[i])
        {
            HeapChunk *garbage = state->heap[i];
            printf("Chunk %p\n", garbage);
            free(garbage);
        }
    }
...
void markReachableChunks(ProgramState *state, HeapChunk *chunk, bool *marked)
{
    // ProgramState *state = createProgramState();
    if (chunk == NULL || marked[chunk - state->heap[0]])
    {
        return;
    }

    marked[chunk - state->heap[0]] = true;

    for (int i = 0; i < chunk->num_references; i++)
    {
        markReachableChunks(state, chunk->references[i], marked);
    }
}
...
//     setVar(state, "a", bChunk);    // a = b (Drawing 3)
    ...
    addReference(aChunk, bChunk); // a->ref = b
    addReference(bChunk, aChunk); // b->ref = a
    // the above line will make the GC loop infinitely if you're not careful!
    HeapChunk *cChunk = HeapMalloc(state);
    setVar(state, "c", cChunk); // c = malloc()
    bChunk = HeapMalloc(state);
    setVar(state, "b", bChunk); // b = malloc()
    setVar(state, "c", bChunk);
    setVar(state, "a", bChunk);
